...
export class PitchTracker {
  detect(buffer, sampleRate) { let rms = 0; for (const value of buffer) rms += value * value; rms = Math.sqrt(rms / buffer.length); if (rms < .004) return { frequency: 0, confidence: 0, level: rms }; const minLag = Math.floor(sampleRate / 1200), maxLag = Math.min(Math.floor(sampleRate / 150), Math.floor(buffer.length / 2)); let bestLag = 0, best = -Infinity; for (let lag = minLag; lag <= maxLag; lag++) { let dot = 0, left = 0, right = 0; for (let i = 0; i < buffer.length - lag; i++) { dot += buffer[i] * buffer[i + lag]; left += buffer[i] * buffer[i]; right += buffer[i + lag] * buffer[i + lag]; } const score = dot / Math.sqrt(left * right + 1e-12); if (score > best) { best = score; bestLag = lag; } } const confidence = Math.min(1, Math.max(0, best)); return { frequency: confidence > .45 ? sampleRate / bestLag : 0, confidence, level: rms }; }
}
